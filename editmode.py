...
from utils import resize_with_aspect_ratio_fill, remove_whitespace
...
class EditMode:
    ...
    def draw_original_on_inpainted(self, event):
        ...
        if 0 <= x_rel < self.inpainted_image.width and 0 <= y_rel < self.inpainted_image.height:
            ...
            original_patch_no_alpha = original_patch_rgba.convert("RGB")
    ...
    def load_original_image(self):
        ...
        if file_path:
            ...
            self.original_scale_factor = min(self.canvas.winfo_width() / self.original_image.width,
                                             self.canvas.winfo_height() / self.original_image.height)
    ...
    def load_inpainted_image(self):
        ...
        if file_path:
            ...
            self.inpainted_scale_factor = min(self.canvas.winfo_width() / self.inpainted_image.width,
                                              self.canvas.winfo_height() / self.inpainted_image.height)
            self.overlay_image = Image.new("RGBA", self.inpainted_image.size, (0, 0, 0, 0))
            self.overlay_draw = ImageDraw.Draw(self.overlay_image)
            self.display_image_on_canvas(self.inpainted_image)
